   ...
   public function index(Request $request)
{
    ...
    $testRequests = TestRequest::query()
        ...
        // Staff can only see test requests from their branch.
        // Admin can see all test requests in the laboratory.
    ...
    public function show(TestRequest $testRequest)
{
    ...
    // Staff can only view test requests from their own branch.
    if (
        auth()->user()->isStaff() &&
        $testRequest->branch_id !== auth()->user()->branch_id
    ) {
        abort(403);
    }
  ...
  public function edit(TestRequest $testRequest)
{
    ...
    // Staff can only edit test requests from their own branch.
    if (
        auth()->user()->isStaff() &&
        $testRequest->branch_id !== auth()->user()->branch_id
    ) {
   ...
   public function update(
    ...
    // Staff can only update test requests from their own branch.
    if (
        auth()->user()->isStaff() &&
        $testRequest->branch_id !== auth()->user()->branch_id
    ) {
   ...
   public function destroy(TestRequest $testRequest)
{
    ...
    // Staff can only delete test requests from their own branch.
    if (
        auth()->user()->isStaff() &&
        $testRequest->branch_id !== auth()->user()->branch_id
    ) {
